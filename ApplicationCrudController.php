...
//        ;
    ...
    )
    {
        ...
        return $this->zipper->getResponseFromZipFiles(
            $filesMap,
            $application->getPracticeId() . '-fascicolo.zip'
        );
    }
    ...
    ): RedirectResponse
    {
        ...
        if ($form->isSubmitted()) {
            ...
            if ($application instanceof Application) {
                ...
                // Nessun attachment nuovo trovato nella collection: il form è stato inviato senza file.
                if ($newAttachment === null) {
                    $this->addFlash('danger', 'Errore! Nessun file trovato nel form. Assicurarsi di aver selezionato un file prima di inviare.');
                    return $this->redirect($redirectUrl);
                }

                /** @var UploadedFile|null $file */
                $file = $newAttachment->getUploadFile();

                // getUploadFile() non ha restituito un file valido.
                if (!$file instanceof UploadedFile) {
                    $this->addFlash('danger', 'Errore! Il file caricato non è valido o il caricamento non è andato a buon fine.');
                    return $this->redirect($redirectUrl);
                }
