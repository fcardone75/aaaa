...
//TODO check field not empty [GDA]
        ...
        this.errorSendFunction = function(data) {
            // data.responseJSON è già un oggetto parsato da jQuery, non serve JSON.parse
            const errorMessage = data.responseJSON;
            const errors = JSON.parse(errorMessage).messageErrors;
            alert('Errore!' + "\n" + errors.join(" \n"));
            that.inputTextMessage.removeClass('loading');
            that.btnSend.prop('disabled', false);
            that.btnSave.prop('disabled', false);
