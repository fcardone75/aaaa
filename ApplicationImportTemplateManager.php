...
class ApplicationImportTemplateManager implements ApplicationImportTemplateManagerInterface
{
    ...
    public function getReferencesSheetForTemplate(ApplicationImportTemplate $applicationImportTemplate): Worksheet
    {
        ...
        if (!$referencesSheet) {
            // Precedentemente Exception. In questo modo puo essere gestita da FileUploadExceptionSubscriber
            throw new RuntimeException(sprintf('The given file does not contain a "%s" worksheet', $this->config['sheet']['name']));
        }
