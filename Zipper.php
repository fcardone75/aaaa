<?php

namespace App\Service;

use App\Service\Contracts\ZipperInterface;
use Gaufrette\FilesystemInterface;
use Symfony\Component\HttpFoundation\Response;
use ZipArchive;

// /application-message-attachments/46/download
class Zipper implements ZipperInterface
{
    public function getResponseFromZipFiles($filesMap, $zipName = 'Documents.zip'): Response
    {
        // usiamo la cartella temporanea definita dalla ENV per evitare problemi con filesystem read-only
        $tempDir = rtrim($_ENV['TEMP_FOLDER_PATH'] ?? sys_get_temp_dir(), DIRECTORY_SEPARATOR);
        $zipPath = $tempDir . DIRECTORY_SEPARATOR . $zipName;

        $zip = new ZipArchive();
        $zip->open($zipPath, ZipArchive::CREATE);

        $missingFiles = [];

        foreach ($filesMap as $fileMap) {
            /** @var FilesystemInterface $fileSystem */
            $fileSystem = $fileMap['fileSystem'];
            $fileKey    = $fileMap['fileKey'];

            try {
                $zip->addFromString(basename($fileKey), $fileSystem->read($fileKey));
            } catch (\Exception $e) {
                $missingFiles[] = $fileKey;
            }
        }

        if (!empty($missingFiles)) {
            $zip->addFromString('missing.txt', implode(PHP_EOL, $missingFiles));
        }

        $zip->close();

        $response = new Response(file_get_contents($zipPath));
        $response->headers->set('Content-Type', 'application/zip');
        $response->headers->set('Content-Disposition', 'attachment;filename="' . $zipName . '"');
        $response->headers->set('Content-length', filesize($zipPath));

        @unlink($zipPath);

        return $response;
    }
}
